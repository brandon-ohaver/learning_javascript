...
// the call to *greet* causes control to jump to the start of the function.
// The function calls *console.log*, which takes control, does its job, and then returns control.
// There it reaches the end of the *greet* function, so it returns to the place that called it.
...
// Every time a function is called, the current context is stored on top of this tack.
// When a function returns, it removes the top context from the stack and uses that context to continue execution.

// Storing this tack requires space in the computer's memory.
// When the stack grows too big, the computer will fail with a message like "out of stack space" or "too much recursion."
// The following code illuestrates this by asking the computer a really hard question that causes an infinite back-and-forth between two functions. 
