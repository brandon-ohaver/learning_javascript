...
// A function that calls itsefl is called *recursive*.
...
// The function calls itself multiple times with every smaller exponents to achieve the repeated multiplication.
...
// It's yet another factor that complicates program design, and when you're doing something that's already difficult, that extra thing to worry about can be paralayzing.

// Therefor, always start by writing something that's correct and easy to understand. 
...
// How would you write a function that, given a number, tries tof ind a sequence of such additions and multiplications that produces that number?
...
// It is okay if you don't see how it works right away.
...
// Finally, if we're still below the target number, the function tries both possible paths that start from the current number by calling itself twice, once for additon and once for multiplication.
// If the first call returns something that is not null, it is returned. Otherwise, the second call is returned, ragardless of whether it produces a string or null.
...
// The first time find is called, it starts by calling itself to expore the solution that start with (1 + 5).
